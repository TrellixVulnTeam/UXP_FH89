...
#ifdef XP_MACOSX
  PanOutputIfNeeded(mGraphImpl->mInputWanted);
#endif
                                  ...
                                  AudioDataValue* aOutputBuffer, long aFrames)
{
  ...
  if (mNeedsPanning && mGraphImpl->AudioChannelCount() == 2) {
  ...
  if (model == MacBookPro && major <= 12) {
    if (cubeb_stream_get_current_device(mAudioStream, &out) == CUBEB_OK) {
      ...
      if (out->output_name && !strcmp(out->output_name, "ispk")) {
        ...
        STREAM_LOG(LogLevel::Debug, ("Using the built-in speakers, with%s audio input",
                              aMicrophoneActive ? "" : "out"));
        mNeedsPanning = aMicrophoneActive;
      } else {
        STREAM_LOG(LogLevel::Debug, ("Using an external output device"));
        mNeedsPanning = false;
      }
